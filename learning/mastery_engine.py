...
class MasteryEngine:
    ...
    def _recompute_skill(self, skill_id: str) -> None:
        """Derive current scores from all evidence using a simple mean.

        v1 deliberately avoids both a permanent high-water mark and an
        uncalibrated forgetting curve. Recency/decay is a later, versioned
        policy once HumanOS has calibration evidence.
        """
    ...
    def record_evidence(self, item: Evidence) -> None:
        ...
        if item.evidence_id in self.evidence:
            if self.evidence[item.evidence_id] == item:
                return  # idempotent retry; do not double-count
    ...
    def snapshot(self) -> dict:
        return {
            ...
            "courses": {k: asdict(v) | {
                "completion_percent": self.course_completion_percent(k),
                "mastery_percent": self.course_mastery_percent(k),
                "metric_status": "ESTIMATED_FROM_RECORDED_EVIDENCE",
                "evidence_count": sum(len(self.skills[s].evidence_ids) for s in v.skill_ids),
            ...
            "policy": {
                "curriculum_self_promotion": False,
                "metrics_execute_external_actions": False,

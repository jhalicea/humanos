...
class HumanOSRuntime:
    ...
    def _host_final(self, tx, hcid, text, response):
        ...
        self.book.checkpoint(tx)
        record_completed_transaction(self.book, tx, hcid, text, response)
        return response
            ...
            try:
                ...
                if text == '/capture-status':
                    ...
                    if args.message is not None:
                        return
                    continue
                if text == '/capture-audit':
                    print('Conversation capture audit: ' + json.dumps(capture_audit(), sort_keys=True), file=sys.stderr)
                    if args.message is not None:
                        return
                    continue
                if text == '/capture':

"""LN-0 V-02: SQLCipher evidence spike for the target HumanOS storage path.

This module is experiment-only. It never opens an owner Notebook. It creates
random-key temporary databases, validates WAL/crash/wrong-key/export behavior,
and proves plaintext SQLite -> new encrypted SQLCipher migration on synthetic data.

It intentionally does not choose final Keychain/KDF/key-rotation policy. The key
wrapper below is only a functional recovery-wrapper proof using the repository's
existing optional ``cryptography`` dependency.
"""
...
def _run_sqlcipher(
    ...
    script: list[str] = []
    if key is not None:
        # SQLCipher requires keying before the first operation that touches the DB.
        script.append(_key_pragma(key))
...
def _require_ok(result: subprocess.CompletedProcess[str], label: str) -> list[str]:
    if result.returncode != 0:
        raise SQLCipherSpikeError(
            f"{label} failed rc={result.returncode}: {result.stderr.strip() or result.stdout.strip()}"
        )
...
def _start_live(
    ...
    proc.stdin.write(f".print {token}\n")
    proc.stdin.flush()
    _wait_for_token(proc, token)
    return proc


def _wait_for_token(proc: subprocess.Popen[str], token: str, timeout: float = 10.0) -> None:
    ...
    while time.monotonic() < deadline:
        if proc.poll() is not None:
            ...
            raise SQLCipherSpikeError(
                f"SQLCipher exited before {token}: rc={proc.returncode} stdout={observed!r} stderr={stderr!r}"
            )
        ...
        if token in line:
            return
    raise SQLCipherSpikeError(f"Timed out waiting for live SQLCipher token {token}; output={observed!r}")
...
def _recovery_wrap_proof(data_key: bytes) -> tuple[bytes, bytes, bytes]:
    ...
    wrapped = AESGCM(recovery_kek).encrypt(nonce, data_key, b"HumanOS-LN0-recovery-wrap-proof-v1")
    recovered = AESGCM(recovery_kek).decrypt(
        nonce, wrapped, b"HumanOS-LN0-recovery-wrap-proof-v1"
    )
    if recovered != data_key:
...
def run_sqlcipher_spike(executable: str | None = None) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="humanos-ln0-sqlcipher-") as tmp:
        ...
        if version < SUPPORTED_MIN or version >= SUPPORTED_MAX_EXCLUSIVE:
            raise SQLCipherSpikeError(
                f"V-02 requires stable SQLCipher >= {SUPPORTED_MIN} and < {SUPPORTED_MAX_EXCLUSIVE}; got {version_text}"
            )
        ...
        # Standard SQLite must not be able to read the encrypted file.
        ...
        if not standard_sqlite_rejected:
            raise SQLCipherSpikeError("Standard SQLite unexpectedly read the SQLCipher database")
        ...
        # Keep a live WAL writer open after an autocommitted insert. The shell token
        # proves the INSERT statement completed before we inspect sidecars and kill it.
        committed_proc = _start_live(
            executable,
            db_path,
            data_key,
            [
        ...
        if not wal_path.exists():
            ...
            raise SQLCipherSpikeError("WAL file was not observable during live WAL transaction")
        ...
        if any(plaintext_scan.values()):
            _kill(committed_proc)
            raise SQLCipherSpikeError(f"Known plaintext found in encrypted DB/WAL/SHM: {plaintext_scan}")
        if db_path.read_bytes().startswith(b"SQLite format 3\x00"):
            _kill(committed_proc)
            raise SQLCipherSpikeError("Encrypted database exposes a plaintext SQLite header")
        _kill(committed_proc)
        ...
        # An open transaction is killed before COMMIT. The marker must not become
        # visible after recovery.
        uncommitted_proc = _start_live(
            executable,
            db_path,
            data_key,
            [
        ...
        # Export an encrypted backup with independent key material.
        ...
        if backup_wrong.returncode == 0:
            raise SQLCipherSpikeError("Encrypted backup accepted the wrong key")
        backup_scan = _scan_for_plaintext(
            [backup_path], [base_marker, committed_marker]
        )
        if any(backup_scan.values()):
            raise SQLCipherSpikeError("Known plaintext found in encrypted backup")

        # Functional independent-recovery wrapper proof. This is deliberately not
        # the final Keychain/KDF design.
        ...
        if _count_value(executable, backup_path, recovered_backup_key, committed_marker, home) != 1:
            raise SQLCipherSpikeError("Recovered wrapped key could not restore encrypted backup")

        # Verify the officially documented plaintext -> NEW encrypted DB migration
        # pattern with sqlcipher_export(). This directly tests the likely safe path
        # for migrating Runtime 0.1 without rewriting its source DB in place.
        ...
        if migrated_lines[-2:] != ["1", "1"]:
            raise SQLCipherSpikeError(
                f"sqlcipher_export did not preserve synthetic data/schema trigger: {migrated_lines!r}"
            )
        ...
        return {
            ...
            "wal_observed": wal_path.exists() or True,
            ...
            "keys_logged": False,
            "note": "Experiment evidence only; final Keychain/KDF/custody policy remains an ADR decision.",
        }

...
class Notebook:
    ...
    def _record_policy_proof(self, activation_seq, integrity_key_id):
        payload = {
    ...
    def _verify_policy(self):
        ...
        if policy != RECORD_INTEGRITY_POLICY or activation is None or proof is None or key_identity is None:
            raise RuntimeError('Record integrity policy metadata is missing or downgraded')
        try:
            activation_seq = int(activation)
        except (TypeError, ValueError) as error:
            raise RuntimeError('Record integrity activation boundary is invalid') from error
        if activation_seq < 0:
            raise RuntimeError('Record integrity activation boundary is invalid')
        expected = self._record_policy_proof(activation_seq, key_identity)
    ...
    def _migrate_record_integrity(self):
        ...
        if not have_integrity:
            ...
            with self._immediate():
                ...
                key_identity = self._meta('integrity_key_id')
                proof = self._record_policy_proof(activation_seq, key_identity)
                self.db.execute(
        ...
        # Existing upgraded state must be internally valid before any missing
        # enforcement trigger is recreated. This avoids hiding a bypass window.
        self._verify_record_state(require_triggers=False)
        triggers = {row[0] for row in self.db.execute(
            "SELECT name FROM sqlite_master WHERE type='trigger'"
        )}
        if REQUIRED_RECORD_TRIGGERS - triggers:
            with self._immediate():
                self._create_record_integrity_triggers()
